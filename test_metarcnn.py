    ...
    for i in range(num_images):
        ...
        for j in range(1, imdb.num_classes):
            ...
            if inds.numel() > 0:
                ...
                if vis:
                    im2show = vis_detections_label_only(im2show, imdb.classes[j], cls_dets.cpu().numpy(), 0.1)
        ...
        if vis:
            if not os.path.isdir(os.path.join('data','VOCdevkit2007','pred')):
                os.mkdir(os.path.join('data', 'VOCdevkit2007','pred'))
            im_dir = os.path.join('data', 'VOCdevkit2007', 'pred', str(imdb.image_index[i])+ '.jpg')
